...
namespace Akka.Persistence.Cassandra.Query
{
    internal class EventsByTagPublisher : ActorPublisher<GuidPersistent>
    {
        ...
        public sealed class ReplayAborted
        {
            ...
            public ReplayAborted(Option<SequenceNumbers> sequenceNumbers, string persistenceId, long expectedSequenceNr,
                long gotSequenceNr)
            {
                SequenceNumbers = sequenceNumbers;
        ...
        public static Props Props(string tag, TimeUuid fromOffset, Option<TimeUuid> toOffset,
        ...
        private Option<long> _abortDeadline;
        private readonly ICancelable _tickTask;
        private long _lookForMissingDeadline;
        private readonly ILoggingAdapter _log = Context.GetLogger();

        public EventsByTagPublisher(string tag, TimeUuid fromOffset, Option<TimeUuid> toOffset,
            CassandraReadJournalConfig settings, ISession session, PreparedStatement preparedSelect)
        {
            Tag = tag;
            ...
            _toOffsetTicks = toOffset.HasValue
                ? toOffset.Value.GetDate().Ticks + _eventualConsistencyDelayTicks
                : long.MaxValue;
            ...
            if (settings.PubsubMinimumInterval.HasValue && settings.PubsubMinimumInterval != Timeout.InfiniteTimeSpan)
            {
                try
                {
                    DistributedPubSub.Get(Context.System).Mediator.Tell(new Subscribe("akka.persistence.cassandra.journal.tag", Self));
                }
                catch
                {
            ...
            _sequenceNumbers = _strictBySequenceNumber
                ? new Option<SequenceNumbers>(SequenceNumbers.Empty)
                : Option<SequenceNumbers>.None;
            _abortDeadline = Option<long>.None;
        ...
        public Option<TimeUuid> ToOffset { get; }
        ...
        private void Replay()
        {
            ...
            if (_log.IsDebugEnabled)
                _log.Debug($"{(backtracking ? "bactracking " : "")}query for tag [{Tag}] from [{_currentTimeBucket}] [{_currentOffset}] limit [{limit}]");
            ...
            return message =>
            {
                ...
                else if (message is ReplayDone)
                {
                    ...
                    if (_currentOffset == _highestOffset)
                        _abortDeadline = Option<long>.None; // back on track again
