...
namespace Akka.Persistence.Cassandra.Query
{
    internal class AllPersistenceIdsPublisher : QueryActorPublisher<string, AllPersistenceIdsState>
    {
        ...
        protected override Tuple<string, AllPersistenceIdsState> UpdateState(AllPersistenceIdsState state,
            Row row)
        {
            ...
            {
                return Tuple.Create((string) null, state);
            }
            return Tuple.Create(@event, new AllPersistenceIdsState(state.KnownPersistenceIds.Add(@event)));
        }
